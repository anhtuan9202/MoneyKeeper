package com.example.moneykeeper.presenter.expense.viewmodel
...
class ExpenseViewModel @Inject constructor(
    ...
    fun calculate(categoryId: Int, month: String, callback: (Int) -> Unit) = viewModelScope.launch(Dispatchers.Main){
        expenseUseCases.getExpensesForMonth(month).collect{
            var total = 0
            for(expense in it){
                if(expense.expCategory == categoryId){
