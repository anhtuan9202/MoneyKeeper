    ...
    private fun handleOnClick(){
        expenseAdapter.setItemClickListener(object : com.example.moneykeeper.presenter.interfaces.OnItemClickListener {
            override fun onItemClick(data: Any?) {
                val itemView = expenseAdapter.getItemView(binding.rvExpense, data as Expense)
                val popupMenu = PopupMenu(requireContext(), itemView)
                popupMenu.inflate(R.menu.popup_menu)
                popupMenu.setOnMenuItemClickListener { menuItem ->
                    when (menuItem.itemId) {
                        R.id.edit -> {
                            callback.showFragment(this::class.java, AddExpenseFragment::class.java,0,0,data,true)
                            return@setOnMenuItemClickListener true
                        }
                        R.id.delete -> {
                            try {
                                expenseViewModel.deleteExpense(data as Expense)
